...
class Converter:
    def __init__(self, file):
        ...
        with open(os.path.dirname(os.path.abspath(__file__)) + file, 'r') as f:
            ...
            self.starts = set(int(x) for x in
                              f.readline().split())  # start points
    ...
    def dfs(self, point):
        ...
        for i in range(self.m):
            new_point = set()  # set состояний, в который можем перейти
            # из этой точки по данному символу
            for elem in point:
                if (self.ends[elem]):
                    self.new_ends.add(index)  # если одна из объединенных
                    # точек была конечной, то новая точка тоже конечная
                for el in self.graph[elem][i]:
                    new_point.add(el)

            if (new_point):  # проверяем, была ли раньше такая непустая точка
                # и добавляем при отсутсвие,
                ...
                # добавляем переход в новую точку
                self.new_graph[index][i].append(new_point_index)
        ...
        with open(os.path.dirname(os.path.abspath(__file__)) + file, 'w') as f:
            ...
            for i in range(len(self.new_graph)):
                for j in range(self.m):
                    if (not self.new_graph[i][j]):
                        continue
                    f.write(str(i) + ' ' + str(j) + ' ' +
                            str(self.new_graph[i][j][0]) + ' ' + '\n')
